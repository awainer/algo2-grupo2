...
/*int TSintactico_Crear(TSintactico* as){
    ...
    P_Crear(&pP, TamanioDato); /creo la pila y le paso el tamanio dl dato que seria enumerado osea int/
    P_Poner(&pP, NADA)          /pongo como primer elemento NADA/
...
int TSintacticoCasoValor(TSintactico * as,Token * token){
    ...
    switch (token->tipo){
		...
		case TOKEN_OBJETO_EMPIEZA:  P_Poner(&pP, OBJETO);
                                    TSintacticoImpimir(as, token);        /*agregar caso al switch para imprimir/
                                    return 0;

		case TOKEN_ARRAY_EMPIEZA:   P_Poner(&pP, ARRAY);
                                    TSintacticoImpimir(as, token);        /*agregar caso al switch para imprimir/
                                    return 0;
		case TOKEN_TRUE:            P_Poner(&pP, VALOR);
                                    TSintacticoImpimir(as,token);        /*agregar caso al switch para imprimir/
                                    return 0;
		case TOKEN_FALSE:           P_Poner(&pP, VALOR);
                                    TSintacticoImpimir(as,token);        /*agregar caso al switch para imprimir/
                                    return 0;

		case TOKEN_NULL:            P_Poner(&pP, VALOR);
                                    TSintacticoImpimir(as,token);        /*agregar caso al switch para imprimir/
                                    return 0;
...
if(EleAux==OBJETO)
{
    ...
    if(token->tipo==TOKEN_STRING)
    {
           ...
           return 0;
            ...
            {
                TSintacticoImpimir(as,token);
                P_Sacar(&pP, &Pe);                                  /saco de la cola el OBJETO/
                P_Sacar(&pP, &Pe);                        /saco de la cola lo que hay en el tope para ver en que estado estoy/
                EleAux=Pe;
                P_Poner(&pP,Pe)
 ...
 if (EleAux== VALOR)
 {
    ...
    else
    {
        ...
        if ((token->tipo==TOKEN_OBJETO_TERMINA) && (EleAux== OBJETO))
        {
            ...
            P_Sacar(&pP, &Pe);
            EleAux=Pe;
            P_Poner(&pP, &Pe);                               /* si viene un objeto_cierra, y antes tenia un objeto, entonces vuelvo una posicion /
            if (EleAux== ARRAY)           /*si antes tenia un array entonces estaba en un valor/
            {
                P_Poner(&pP, VALOR);
